...
- 订阅 `claude`（Anthropic OAuth）已移除：`ccl oauth` 只接受 gpt|gemini|grok|copilot|qoder|kimi|kiro|workbuddy|commandcode（`ValidateLoginProvider`），但旧配置/凭据文件里的 `chatgpt` 值仍可读取；Command Code 的浏览器登录即官方 CLI login 的模拟（回调 + 粘贴双路径），非浏览器替代是 `ccl import commandcode`（`ImportCredential`）。

`internal/oauthproxy/doc.go` 是数据面归属的权威清单（Codex Responses、Copilot、Qoder、Kiro、WorkBuddy、native passthrough、会话凭据约束），把它当作回归 checklist，保持所有 provider 由 CCL 自有 runtime 处理。所有 runtime 只绑定 `127.0.0.1` 并使用每会话随机 key；`Runtime.Stop` 先取消并等待 serve 退出，超时后才强制 Shutdown。错误恢复分两层：所有数据面（`internal/oauthproxy/retry.go` 的 `retryUpstream`）对上游 429/5xx 先快速重试两次（500ms/1s，不遵循上游 `Retry-After`），预算耗尽后把原始 status/响应体/`Retry-After` 原样透传，交 Claude Code 自己退避；各 provider 的恢复逻辑属于一次 attempt 的一部分：Codex/Grok/Kimi 401 后刷新一次，WorkBuddy 401/403 后刷新一次，Gemini 网络错误/429/5xx 回退控制面 base（一次 attempt 最多 2 次上游调用），Qoder 按凭据轮换。三层刻意豁免：Kiro 用自有 1/2/4s 预算 + 每轮凭据轮换（叠加会双倍退避）；WorkBuddy/Copilot 的 loopback 网关是两跳结构的内层跳，快速重试归外层 service 所有，内层再包会 3×3=9 次。Command Code 的 402（计费态，映射为 429 给客户端）向重试层上报原始 status，不烧预算。
