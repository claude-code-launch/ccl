...
// reconcileOAuthDefault clears a generated default that disappeared from the
// live catalog. When the value is a known default from an older CCL release and
// the new preferred model is available, it migrates the slot in place. User
// values outside this small generated-default set remain untouched.
func reconcileOAuthDefault(configured *string, oauthProvider, slot, preferred string, availableModels []string) {
	if configured == nil || !isGeneratedOAuthDefault(*configured, oauthProvider, slot, preferred) {
		return
	}
	// A value generated by an older CCL release follows the new default when it
	// is present in the account catalog, even if the old model is still offered.
	// This is what moves an existing Grok provider from 4.5 to 4.6.
	if isLegacyOAuthDefault(*configured, oauthProvider, slot) && !isPreferredDefault(*configured, preferred) && modelListContains(availableModels, preferred) {
	...
	if modelListContains(availableModels, preferred) {
		*configured = preferred
		return
	}
	*configured = ""
...
func isGeneratedOAuthDefault(configured, oauthProvider, slot, preferred string) bool {
	if isPreferredDefault(configured, preferred) {
		return true
	}
	return isLegacyOAuthDefault(configured, oauthProvider, slot)
}

func isLegacyOAuthDefault(configured, oauthProvider, slot string) bool {
	if !strings.EqualFold(strings.TrimSpace(oauthProvider), "grok") {
		return false
	}
	...
	for _, candidate := range legacy[slot] {
		if isPreferredDefault(configured, candidate) {
			return true
		}
	}
	return false
