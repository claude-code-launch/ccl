...
// detectProtocolAndModels tries Anthropic first, then OpenAI.
// Returns (protocol, comma-separated-models, error). error is non-nil when
// both API calls fail (endpoint unreachable or auth rejected); the returned
// protocol is then just a URL-based guess and models is empty.
func detectProtocolAndModels(endpoint, apiKey string) (string, string, error) {
	...
	return guess, "", fmt.Errorf(locale.T(
