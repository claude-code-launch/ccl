...
Examples:
  ...
  ccl oauth group rm grok1`,
	...
	cmd.Flags().StringVar(&opts.oauthProvider, "provider", "", "OAuth backend for a new group (gpt, gemini, grok, copilot, kimi, claude)")
...
func inferGroupProvider(name string, credentials []oauthproxy.CredentialInfo) string {
	...
	for _, candidate := range []string{"grok", "gemini", "copilot", "kimi", "claude", "gpt"} {
